...
with predictor.graph.as_default():
    ...
    while m < M0:
        ...
        # normalize by volume of each second
        r_m_part = r_m.reshape((n_frame, n_bin*n_ch))
        r_m_part = np.concatenate([r_m_part, np.zeros((n_pad, n_bin*n_ch), dtype=np.float32)])
        r_m_part = r_m_part.reshape((secs, frame_per_second*n_bin*n_ch))
        r_m_amp = np.max(np.abs(r_m_part), axis=1)
        r_m_part *= avg_mag.reshape((-1,1)) / np.max(avg_mag)
        r_m = r_m_part.reshape((-1, n_bin, n_ch))
        r_m = r_m[:-n_pad]
        r_m = r_m.reshape((X.shape[0], X.shape[1], n_bin, n_ch))
        ...
        perturb_mag = np.linalg.norm(diff, axis=1)
        ...
        if np.any(perturb_mag > avg_mag * perturb_allowance):
            break
        X = X_next
        m += 1
